...
5. Additional browser IDs can be created in the same helper when their profile ID and paths match the process profile. Navigation, resize, focus, visibility, pointer and keyboard input, reload, developer-tools, cookie, and close operations route to the addressed browser.
