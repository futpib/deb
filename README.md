...
`deb` loads unpacked native extensions at profile-engine startup while Chromium remains an Alloy windowless browser. The extension code runs inside the selected engine's own implementation: Chromium uses its Manifest V3 service workers, content scripts, storage, declarative network rules, and extension APIs; Firefox uses Gecko's WebExtension manager, background documents, content scripts, storage, request rules, and APIs. The shell does not evaluate extension JavaScript or proxy extension network requests.

The XDG directory layout is the extension configuration:

```text
$XDG_DATA_HOME/deb/extensions/all/
  chromium/<extension>/manifest.json
  firefox/<extension>/manifest.json

$XDG_DATA_HOME/deb/profiles/<profile-id>/extensions/
  chromium/<extension>/manifest.json
  firefox/<extension>/manifest.json
```

Immediate child directories under `extensions/all/<engine>` are enabled in every profile. Children under a profile's `extensions/<engine>` are enabled only in that profile and override global entries with the same native extension ID. Chromium IDs are derived from a manifest `key`, Firefox IDs use `browser_specific_settings.gecko.id` or the legacy equivalent, and the directory name is the installation key when a manifest has no explicit native ID. Entries are resolved in deterministic ID order. Dot-directories, ordinary files, and the `disabled/` directory are ignored; active directories are required to contain a parseable manifest. Symlinks are supported, so several profile scopes can select one installation without copying it.

Install or update an extension by atomically placing or replacing its unpacked directory. Disable it by moving it below the adjacent `disabled/` directory, and remove it by moving it to trash or deleting it. The resolved set is fixed when that profile's engine helper starts, so changing it currently requires restarting the affected helper or application. Extension-owned data remains in each engine's existing per-profile data directory even when the code comes from the global directory.
...
`--load-extension /absolute/path` loads the same unpacked directory into both engines. Every option is repeatable. Paths are canonicalized, manifests are validated, and extension options are consumed by deb before Qt processes its own command line. CLI entries apply to every profile and override matching XDG entries, which keeps them useful for development without creating another persistent registry. Chromium registrations last for the helper process and use its persistent profile for extension-owned data. Firefox installs these packages through Gecko's temporary-addon API on each start while keeping their native profile storage.
