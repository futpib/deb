...
- navigation, reload, focus, resize, pointer and keyboard input, page cursor changes, HTML fullscreen entry/exit, engine-native developer tools, and close
...
- Engine-created popup browsers, downloads, extensions, embedded web-content accessibility integration, full IME preedit/composition handling, arbitrary application-defined custom schemes, per-request-context cookie managers, and request interception are outside the current CEF slice. The Qt shell itself exposes production accessibility IDs, roles, names, states, and geometry through AT-SPI. Normal key events and committed input-method text are supported.
